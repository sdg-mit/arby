...
module ArbyModels::AlloySample::Puzzles
  ...
  # initially stacked in decreasing size on one of three pegs. The
  # objective is to transfer the entire tower to one of the other
  # pegs, moving only one disk at a time and never a larger one onto a
  # smaller.
  #
  # The Alloy model below is written so that a solution to the model
  # is a complete sequence of valid moves solving an instance of the
  # problem.  We define constraints for the initial state (all discs
  # on left stake), the final state (all discs on right stake), and
  # each pair of adjacent states (the top disc is moved from one stake
  # to another, not putting larger discs on smaller discs), and let
  # Alloy Analyzer solve for the sequence of states satisfying these
  # constraints.  Since each adjacent pair of states is constrained to
  # be related by a single move, it is easy to see the sequence of
  # moves once you have the sequence of states.
  #
  # For n discs, 2^n states are needed for a solution (including the
  #   initial state and the final state).
  #
  # Performance: currently, the problem can be solved for up to 5
  # discs; this takes several minutes with the Chaff solver.
  #
  ...
  alloy :Hanoi do
    ...
    fun topDisc[st: State, stake: Stake][lone Disc] {
      st.discsOnStake(stake).select{|d| st.discsOnStake(stake).in? Disc::nexts[d] + d }
    }
    ...
    pred game1 {
      Disc.in? State::first.discsOnStake(Stake::first) and
      some(finalState: State){ Disc.in? finalState.discsOnStake(Stake::last) } and

      # each adjacent pair of states are related by a valid move of one disc
      all(preState: State - State::last) {
        let(postState: State::next[preState]) {
          some(fromStake: Stake) {
             # must have at least one disk on fromStake to be able to move
             # a disc from fromStake to toStake
             some preState.discsOnStake(fromStake) and
        ...
        let(postState: State::next[preState]) {
          some(fromStake: Stake) {
            let(d: preState.topDisc(fromStake)) {
              ...
              # a disc from fromStake to toStake
              some preState.discsOnStake(fromStake) and
              postState.discsOnStake(fromStake) == preState.discsOnStake(fromStake) - d and
              some(toStake: Stake) {
                # post- results from pre- by making one disc move
                preState.discsOnStake(toStake).in? Disc::nexts[d] and
                postState.discsOnStake(toStake) == preState.discsOnStake(toStake) + d and
                # the remaining stake afterwards has exactly the discs it had before
                let(otherStake: Stake - fromStake - toStake) {
    ...
    run :game1, 1, Stake => 3, Disc => 5, State => 32 # expect sat
    run :game2, 1, Stake => 3, Disc => 3, State => 8 # expect sat
  end
end
